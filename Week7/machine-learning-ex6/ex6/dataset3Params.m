...
for testC = [0.01, 0.03, 0.1, 0.3, 1, 3, 10, 30]
    for testSigma = [0.01, 0.03, 0.1, 0.3, 1, 3, 10, 30]
		...
		fprintf('Training count %d.\n', testCount);
        model = svmTrain(X, y, testC, @(x1, x2) gaussianKernel(x1, x2, testSigma));
        predictions = svmPredict(model, Xval);
        prediction_error = mean(double(predictions ~= yval));
        results(testCount,:) = [testC, testSigma, prediction_error];     
    end
end

sorted_results = sortrows(results, 3); % sort matrix by column prediction error, ascending
